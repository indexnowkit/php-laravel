 ...
 * when {@see SitemapSupport::installed()} holds. An application replaces `SitemapSourceInterface` to read from
 * another place or format ([docs/extending.md]).
 */
final class SitemapServices
{
    ...
    public static function register(Container $app, string $logger): void
    {
        ...
        $app->singleton(SitemapConfig::class, static function (Container $app) use ($logger): SitemapConfig {
            try {
                return SitemapConfig::fromArray(self::block($app));
            } catch (ConfigurationException $e) {
                $app->make($logger)->critical('indexnow: invalid sitemap configuration, the sitemap command is disabled until it is fixed: {error} (run "php artisan indexnow:check")', ['error' => $e->getMessage(), 'exception' => $e]);
