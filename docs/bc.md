...
| **Artisan commands and options** (`indexnow:check`, `indexnow:key:generate`, `indexnow:submit`, `indexnow:submit-model`, `indexnow:explain`, `indexnow:sitemap`, `indexnow:history`, `indexnow:status`) | Names, arguments and options come from `Console\Definitions` of `indexnowkit/console` (and the sitemap and history packages); new options are only added. The `model` argument of `submit-model` and `explain` stays. Output is not a contract except the exit codes and the `--json` shape of the core formatter. The classes are the packages' since 0.15.0 — `IndexNowKit\Console\Command\*`, `Sitemap\Console\SitemapCommand`, `History\Console\{History,Status}Command` — each bound in the container under its class name ([extending.md](extending.md) "Replacing a command"). |
...
| **Check classes** `Check\QueueCheck`, `Check\EloquentCheck`, `Check\CacheStoreProbe` and the `indexnowkit.check` tag | Names stay; adding a tagged `CheckInterface` keeps working. A tagged service of another type is a `ConfigurationException`. The `router.locales` line is the core's `Check\LocalesCheck` since 0.15.0 (`Check\RouterCheck` of this package is gone: same code, same texts). |
